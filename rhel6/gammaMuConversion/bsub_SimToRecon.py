...
def main(options,args): 
	...
	for i,fn in enumerate(listOfRootFiles):
		...
		f1.write('cp %s/%s ${__LSF_JOB_TMPDIR__}/current_ldmx_sim_events.root \n' % (options.idir,fn));
		f1.write('cd ${__LSF_JOB_TMPDIR__} \n');
		f1.write('pwd \n');
		f1.write('tar -xvzf inputs.tar.gz \n');
		f1.write('ls -l \n');
		f1.write("ldmx-app fullRecoConfig.py \n");
		f1.write("mv ldmx_recon_events.root ${LSB_OUTDIR}/recon_%s \n" % (fn));
		f1.close();
