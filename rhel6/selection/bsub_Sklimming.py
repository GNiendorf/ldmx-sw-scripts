...
def main(options,args): 
	...
	os.system('tar -cvzf inputs.tar.gz sklimmer.py ../tdrstyle.py');	
	...
	for i,fnf in enumerate(listOfRootFiles):
		...
		f1.write('ls -l \n');
		f1.write("python sklimmer.py -b -i %s.root -o sklim_%s.root --swdir %s --tag %i \n" % (fn,fn,swdir,i));
		f1.write("mv sklim_%s.root ${LSB_OUTDIR}/sklim_%i_%s.root \n" % (fn,i,fn));
		f1.close();
